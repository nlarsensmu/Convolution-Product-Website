...
var runImg = function(iJimpSrc) {
    ...
    const NOTIFICATION_BODY = `Running ${selectedFilter.value} took ${(t2 - t1)/1000} seconds`
    let notif = new Notification(NOTIFICATION_TITLE, { body: NOTIFICATION_BODY })
}
...
save_btn.addEventListener('click', () => {
    ...
    const image = document.getElementById('output_img');

    var jimpSrc = Jimp.read(image.src);
    
    jimpSrc.then(x => {
        console.log("settitle renderer.js")
        console.log(x)
        const folder = document.getElementById('save_location').value
        var img = new Uint8Array(x.bitmap.data);
        window.electronAPI.saveImg(folder, img, x.bitmap.height, x.bitmap.width)
    })
});